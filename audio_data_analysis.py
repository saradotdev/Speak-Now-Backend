...
def preprocess_audio(file_path, target_shape=(216, 1)):
    ...
    features = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=13, hop_length=512) # extract features

    features_flat = features.flatten('F')  # flatten the features along the last axis ('F' means flatten in column-major order)

    features_flat = features_flat[:np.prod(target_shape)]  # truncate if larger
    features_flat = np.pad(features_flat, (0, np.prod(target_shape) - len(features_flat)))

    features_flat = (features_flat - np.mean(features_flat)) / np.std(features_flat) # normalize features 
...
def predict_emotion(model, features):
    ...
    features = np.reshape(features, (-1, 216, 1)) # reshape features to match the model's expected input shape
...
emotion_labels = ["female_angry", "female_calm", "female_fearful", "female_happy", "female_sad", "male_angry", "male_calm", "male_fearful", "male_happy", "male_sad"]
